...
- **Terminal device IDs**: list devices via `terminals.listDevices`, persist user-friendly names. Choose device per checkout.
...
- **Webhook fallback**: open Checkout pages poll `/api/square/terminal-checkout/{id}` every 5s while any payment is `pending`. If staff closes the tab, the payment remains `pending` until someone reopens the ticket. No server-side cron sweep in v1.
