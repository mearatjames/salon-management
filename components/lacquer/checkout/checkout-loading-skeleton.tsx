// CheckoutLoadingSkeleton — shared shell used by both checkout loading routes:
//   - app/(studio)/checkout/loading.tsx
//   - app/(studio)/checkout/[ticketId]/loading.tsx
//
// Pure server component (no "use client"). Mirrors the live page's
// `.checkout-shell` chrome: the `.checkout-header` band, the
// `.checkout-tech-band` row, and the two-column `.checkout-body`
// (catalog column on the left, cart/summary column on the right).
//
// Every placeholder uses the shimmer <Skeleton> primitive (styles/loading.css).
