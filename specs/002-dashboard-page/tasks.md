...
- [ ] T004 Create `lib/auth/session.ts` exporting `type StudioViewer = { id: string; staffId: string; displayName: string }` and `export async function requireStudioSession(): Promise<StudioViewer>` that returns the fixed stub `{ id: "demo", staffId: "maya", displayName: "Maya Patel" }`. Top of the file documents (single short comment) that the body is replaced by the auth feature; the signature is the contract. (research.md R2, plan.md Complexity Tracking row 3.)
- [ ] T005 [P] Create `lib/dashboard/mock-data.ts` — a TypeScript port of `design-system/prototypes/transaction/data.jsx`. Export `Service`, `Technician`, `TxLineItem`, `Transaction` types matching `data-model.md`, plus `SERVICES`, `STAFF`, `TX_HISTORY` (every row verbatim from the prototype, in the same order), and `PERIOD_FACTOR = { today: 1, week: 6.4, month: 27 }`. Drop the `window.*` assignments. Top-of-file `// TODO` comment cites the Supabase-wiring feature as the replacement point. (research.md R1, R9.)
- [ ] T006 [P] Create `lib/dashboard/format.ts` exporting `formatCurrency(amount)`, `formatPercent(fraction)`, `formatCount(n)`, `formatServiceLabel(items, services)`, and `paymentMixWidths(byMethod, total)` exactly as specified in `contracts/dashboard-data.contract.md`. Use `Intl.NumberFormat("en-US", …)` for currency / percent / count. `paymentMixWidths` returns `{ card, cash, gift, neutral }` percentages — `{0,0,0,100}` when `total === 0` (FR-018 zero branch).
- [ ] T007 [P] Create `tests/unit/dashboard/format.test.ts` covering: integer currency rendering with comma thousands (`$1,240`), zero currency (`$0`), percent rounding (`0.184 → "18%"`, `0.185 → "19%"`), `formatServiceLabel` ≤2-items (`"A, B"`) and ≥3-items (`"A +2 more"`) cases, and `paymentMixWidths` happy-path + zero-total branch. Run `npm test -- format` to confirm green.
- [ ] T008 Create `lib/dashboard/aggregate.ts` exporting `txTotals(tx)`, `txAggregate(list)`, `applyPeriodFactor(base, period)`, and `buildDashboardData()`. Mirror the prototype math from `design-system/prototypes/transaction/data.jsx:183-209`. `applyPeriodFactor` rounds `count` and `services` to integers and short-circuits to all-zeroes when `base.count === 0`. `buildDashboardData()` is pure: reads `TX_HISTORY` / `STAFF`, precomputes all three `DashboardSummary`s, builds the 7-row `TransactionRow[]` (most-recent-first using `formatServiceLabel`), attaches the static `QuickAction[]` and `comparisons` object (populated only for `today`), and returns the `DashboardData` shape from `data-model.md`. (Depends on T005, T006.)
- [ ] T009 Create `tests/unit/dashboard/aggregate.test.ts` covering: `txTotals` for a single-item and a multi-item transaction (incl. add-on with explicit `price` override), `txAggregate` totals invariant (`byMethod.card + .cash + .gift ≈ total`), `applyPeriodFactor` integer rounding (`count`, `services` are integers for every period), FR-018 zero-count short-circuit, and `buildDashboardData()` end-to-end: `summaries.today.count === 17`, `summaries.week.count === Math.round(17 * 6.4)`, `summaries.month.count === Math.round(17 * 27)`, `recent.length === 7`, `recent[0].id === "tx-0130"` (most recent), `recent[6].id === "tx-0124"`, and `comparisons === null` only when invoked with `period === "today"` filtered out (the function returns comparisons unconditionally but `<PeriodSummary />` will hide them — assert the shape, not the rendering). Run `npm test -- aggregate` to confirm green. (Depends on T008.)
- [ ] T010 Create `app/(studio)/layout.tsx` — a minimal studio shell. Render `<html>`-less (root layout owns it); the studio segment wraps its children in a centered `<main className="tx-landing" data-density="regular">` container that imports `styles/dashboard.css` (top of file). Add visible-but-disabled header controls "Switch staff" (anchor with `aria-disabled`) and a "Reconnecting…" banner placeholder (CSS-hidden by default). One short comment notes both are wired by later features. (plan.md Complexity Tracking row 2, quickstart.md Known placeholders.)
- [ ] T011 Replace `app/page.tsx` with a Server Component that calls `redirect("/dashboard")` from `next/navigation`. Remove the placeholder JSX entirely. After this task, `npm run dev` + opening `localhost:3000` should issue a 307 to `/dashboard` (which returns 404 until T017). (FR-001, SC-005.)
...
- [ ] T014 [P] [US1] Create `components/lacquer/period-toggle.tsx` as a `"use client"` component matching the contract: `value`, `onChange`. Three `<button>` elements styled via `.tx-period` + `.active`. The `onChange` handler short-circuits when `next === value` (Edge case "Period switch during slow render"). Each button is a native `<button>` so focus-visible inherits from the shadcn `button` primitive imported as a base. (FR-004.)
- [ ] T015 [US1] Create `components/lacquer/period-summary.client.tsx` as a `"use client"` component matching the contract: `summaries: Record<DashboardPeriod, DashboardSummary>`, `comparisons`. Holds the active-period state via `useState<DashboardPeriod>("today")`. Renders a flex/grid wrapper containing four `<StatCard />`s and one `<PaymentMixCard />`, reading values from `summaries[period]`. Passes `comparisons.transactionsVsAvg` to the Transactions card's `delta` and `comparisons.revenueDelta` to the Revenue card's `delta` **only when `period === "today"`** (FR-006); pass `null` otherwise. Mounts `<PeriodToggle value={period} onChange={setPeriod} />` and returns it through a `toggle` render prop or a small context so the page header can place it visually next to the eyebrow without rendering it twice. (Depends on T012, T013, T014.)
- [ ] T016 [US1] Create `app/(studio)/dashboard/page.tsx` as a React Server Component. Top of file: `import "@/styles/dashboard.css"` (idempotent — the studio layout also imports it, but this anchors the page to the stylesheet). Call `await requireStudioSession()` (T004) and `buildDashboardData()` (T008). Render the `<div className="tx-landing">` chrome containing `<div className="tx-landing-top">` with two columns: left column = eyebrow + `<h1>Today at the salon</h1>` + subtitle (use `data.greeting`); right column = `<PeriodSummary />`'s exposed toggle (placeholder — leave a `<div id="period-toggle-slot" />` for now; T020 inserts the CTA next to it). Below the header: render `<PeriodSummary summaries={data.summaries} comparisons={data.comparisons} />` inside a 6-column grid wrapper that lays out 4 stat cards + the payment-mix card spanning 2 columns. Leave the lower split empty for now (T027 fills it). (FR-002, FR-003.)
- [ ] T017 [US1] Create `tests/e2e/dashboard.spec.ts` (Playwright). Cover US1 only: navigate to `/dashboard`; assert the eyebrow, `<h1>`, and subtitle text from FR-003; assert the five tile labels are present (`Transactions`, `Services`, `Revenue`, `Tips`, `Payment mix`); assert the Today-default values (`17` transactions; `~$1,*** ` revenue; assert a deterministic value via the same constants the unit tests use — read them from `lib/dashboard/aggregate.ts` via a small helper export). Click `Week` and assert every tile value changes; click `Month` and assert every tile value changes again and `+3 vs avg` / `+12%` are NOT in the DOM (FR-006). Click `Today` again and assert deltas reappear. Re-click the already-active button and assert no value flicker (re-clicking is a no-op). Add a `page.route` listener and assert no `fetch` / XHR fires during the toggle (SC-003). (FR-004, FR-006, SC-003.)
...
- [ ] T026 [US3] Edit `app/(studio)/dashboard/page.tsx` to add the lower split — a CSS grid with `gridTemplateColumns: "1fr 1.6fr"`, gap 16, taking the remaining vertical space. Left column: a flex-column containing a `.muted` "Quick actions" eyebrow + `<SecondaryActions actions={data.quickActions} cols={1} />`, then a `.muted` "Techs on shift" eyebrow + `<TechsOnShiftTile staff={data.staff} />`. Right column: `<RecentTransactionsFeed rows={data.recent} />`. (Depends on T023, T024, T025.)
- [ ] T027 [US3] Extend `tests/e2e/dashboard.spec.ts` with a US3 test block: assert exactly 4 `.tx-secondary-action` buttons with the labels and hints from the spec; click each and assert URL matches its `href` (placeholder routes 404 — that is fine, the navigation must fire); assert the techs-on-shift tile contains exactly 8 `<TechAvatar />` siblings (matching `STAFF.length`); assert `.tx-feed-row` count is exactly 7; assert `.tx-feed-row` order — first row is `tx-0130` (4:14 PM Walk-in), last visible row is `tx-0124`; assert at least one `.svc` cell ends with the FR-012 `+N more` suffix (tx-0117 has 2 items so it does *not*; pick one with ≥3 once you add to TX_HISTORY — TX_HISTORY ships as-is, so this assertion uses `formatServiceLabel` on a fixture in the test rather than the rendered DOM, or assert the matching pattern is reachable via at least one item count). If no transaction in the canonical `TX_HISTORY` has ≥3 items, change the assertion to: assert the `formatServiceLabel` *function* renders `+N more` for the 3-item case via a Node-side smoke test inside the spec (acceptable Playwright pattern: a `test.beforeAll` import + `expect` on the pure helper). (FR-009, FR-010, FR-011, FR-012.)
...
- [ ] T028 [P] Add a `@media (max-width: 720px)` block to `styles/dashboard.css` that: (a) collapses the stat grid to `gridTemplateColumns: repeat(2, 1fr)`, (b) collapses the lower split to `gridTemplateColumns: 1fr`, (c) shrinks `.tx-landing-top` to a single stacked column, (d) reduces `.tx-stat-card .val` font-size to `22px`. Verify in DevTools at 360 px, 720 px (boundary), 1024 px (tablet), and 1440 px that no horizontal scrollbar appears and no tile content overlaps. (FR-019, SC-006.)
- [ ] T029 [P] Run the design-system auditor against `/dashboard`: invoke the `speckit-design-auditor` agent with target route `app/(studio)/dashboard/page.tsx` and the prototype reference `design-system/prototypes/transaction/Landing.jsx` lines 282–372. Address every reported violation. (SC-004, Constitution Principle I.)
- [ ] T030 Run all quality gates locally: `npm run typecheck && npm run lint && npm test && npm run test:e2e`. All four MUST pass. Resolve any drift before requesting review. (Constitution Principle IV, Development Workflow & Quality Gates.)
