...
- [ ] T020 Create `app/(studio)/settings/policy/actions.ts` with `createSupplyType` per `contracts/server-actions.contract.md § 1`. Imports `requireStudioSession`, `createSupabaseServiceRoleClient`, `recordAudit`, `assertCanWriteCatalog`, `validateSupplyTypeName`, `revalidateSupplyTypeConsumers` (T024). Parses `name`, `return_to` (default `'edit_policy'`), `selected_service_id` (optional). On PG `23505` from the partial unique index, map to `?error=name_taken`. Awaits `recordAudit('supply_type.created', …, { name })`. Redirects per the `return_to` switch in the contract (default → `/services?policy=open&toast=supply_type_created&name=…`; `service_picker` → `/services?selected=<sid>&supply_type_id=<new>&toast=…`).
...
- [ ] T028 [P] [US1] Create `components/lacquer/services/supply-type-picker.client.tsx` per `contracts/ui.contract.md § 4`. Composition: shadcn `Popover` + `Command`. Renders trigger button (selected type name OR "Pick a supply type" placeholder + `ChevronDown`). Dropdown: `CommandInput` placeholder "Search supply types…", `CommandList` of active types with `Check` icon on the selected, plus pinned `CommandItem` at the bottom for inline-create. Inline-create state: single `<Input>` + Save / Cancel buttons; soft-hint mode swaps Save for "Select existing" when `canonicalizeName(typed)` matches an active type's `name_canonical`. Emits hidden `<input type="hidden" name="supply_type_id" value={selectedId ?? ''}>` for FormData submission. Props match the contract: `{ types, selectedId, onSelect, disabled, serviceId }`.
...
- [ ] T030 [US1] Edit `app/(studio)/services/page.tsx`: load the supply-types catalog via `loadSupplyTypesCatalog()` (from T016) and pass `catalog.active` down to `<EditPanel>` → `<ServiceForm>` → `<DeductionsSection>` → `<SupplyTypePicker>` as the `supplyTypes` prop. Read the `?supply_type_id=<id>` URL param (set by `createSupplyType`'s `return_to=service_picker` redirect) and apply it as the draft buffer's initial `supply_type_id` so the picker opens pre-selected on the just-created type.
