...
- The Edit Policy sheet is the right home for the supply-types management section (per `design-system/prototypes/services/EditPolicySheet.jsx`). 021 deferred the sheet shell itself to Phase 2 — it does NOT exist in code yet. This feature therefore ships **both** (a) the minimal Edit Policy sheet shell (right-side sheet with scrim/animation, opened from a new "Edit policy" button in the services page header) and (b) the "Supply types" section inside it. The card-fee defaults section, payment-method matrix, and other Phase-2 policy content stay deferred; the sheet renders only the Supply Types section in this feature. No standalone settings sub-page is created for the catalog.
