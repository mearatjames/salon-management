...
| **V. Scope Discipline & Cost Restraint** | ⚠️ Documented deviation | Constitution V lists "payroll reporting" as a **deferred** v1 item. This feature was explicitly, directly requested by the maintainer through `/speckit-specify` — which the constitution's own Governance clause and Principle V both recognize as a valid scope change ("an explicit, documented scope change approved by the maintainer"). Recorded in Complexity Tracking. Scope is held minimal: no pay-stub emailing, no cash-drawer integration (both deferred in the spec). No new paid infrastructure. **Recommendation: run `/speckit-constitution` to remove "payroll reporting" from the Principle V deferred list so the docs stay honest.** |

**Gate result**: PASS. The single deviation (Principle V) is justified and documented; no unjustified violations.
...
|-----------|------------|--------------------------------------|
| Adds "payroll reporting" — a Constitution V **deferred** v1 item | The maintainer directly and explicitly requested the Payroll page via `/speckit-specify`, with a design handoff to implement. Constitution V and Governance both permit a deferred item to be added by "an explicit, documented scope change approved by the maintainer." | Not building it is not an option — it is the requested feature. The deviation is recorded here; the recommended cleanup is a `/speckit-constitution` amendment removing the item from the deferred list. Scope is otherwise held minimal (no pay-stub email, no cash-drawer coupling, no new paid infra). |
| New tables `pay_periods` + `payroll_payouts` (rather than computing payroll purely on the fly) | The spec's clarified decision is a **full system of record** — durable periods, payouts, closes, and history that survive reload (FR-025, FR-029, FR-031, SC-004/SC-006). Snapshots are also a Constitution III requirement (historical records snapshotted). | A pure read-only computed view was offered to the maintainer and explicitly rejected during `/speckit-clarify`. Without persistence there is no "paid" state, no immutable closed period, and no history. |
