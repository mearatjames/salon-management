...
| **III — Auditability & Money Integrity** | Read-only — no writes, no money mutations, no audit rows. Displayed money is server-authoritative: subtotal/tax come straight from `tickets.*_cents`, tips from `payments.tip_cents`; the client never recomputes monetary truth, only formats it. | PASS |
...
| **V — Scope Discipline & Cost Restraint** | Zero new schema, zero new dependencies, zero new paid infrastructure. Refund/void and CSV export stay deferred (out of scope per spec). Tax is displayed as the stored `$0` — no tax compute path is added. Reuses existing query/format helpers wherever they fit. | PASS |
