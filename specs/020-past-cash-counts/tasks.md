...
- [ ] T027 [US2] US2 phase gate: `npx prettier --check $(git diff --name-only --diff-filter=ACMR HEAD) && npx eslint $(git diff --name-only --diff-filter=ACMR HEAD | grep -E '\.(ts|tsx|js|jsx)$' || echo .) && npm run typecheck && npm test && npx playwright test tests/e2e/past-cash-counts.spec.ts -g "US2"`.
