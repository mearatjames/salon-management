...
- Q: The ServicesV1 prototype includes an X (Close) icon button in the panel header. In a two-pane (always-visible) layout, what should it do? → A: **Omit it entirely.** The panel always shows the current selection or its empty state; deselection happens by clicking a different row or "Add service." No third "panel collapsed by close gesture" state, no third dialog trigger. Matches always-visible inspector panels in Linear / Figma side panels.
- Q: Should the custom card-fee amount and the supply deduction amount have an explicit upper cap (validator + DB CHECK constraint)? → A: **Cap both at $50 (5000 cents).** Guards against fat-finger errors ($300 instead of $3.00, $5000 instead of $5.00) while still covering any realistic salon supply cost (chrome, GelX tips, OPI bottle wear all well under $20). The cap applies only to user-typed `card_fee_custom_cents` and `supply_amount_cents`; it does NOT constrain `price_cents` or any of the variable-price bounds (those have no cap and inherit the existing 008 validation rules).
...
4. **Given** Custom is selected with an empty amount input, **When** the owner attempts to save, **Then** the input shows an inline hint and "Save changes" stays disabled; allowed values are `$0` or any positive amount up to `$50` (an explicit `$0` is permitted because it means "intentionally zero for this service" rather than "exempt from card-fee logic entirely"). If the typed value exceeds `$50`, the input shows "Card fee can't exceed $50." and Save stays disabled.
...
3. **Given** Supply is on with an amount of `$0` (or empty), **When** the owner tries to save, **Then** the amount input shows an inline hint ("Enter a positive amount up to $50, or turn Supply off") and "Save changes" stays disabled; an explicit `$0` is rejected because Supply-on with no money is a contradiction (the toggle off state is how you say "no supply for this service"). A value above `$50` is also rejected with the hint "Supply can't exceed $50."
...
- **FR-002**: The right pane MUST replace the existing drawer overlay from feature 008. The drawer component and its backdrop / Escape close behavior MUST be removed from this surface (drawer-style overlays remain valid for unrelated surfaces). The panel header MUST NOT include a Close (X) affordance — the panel is always visible and is "deselected" only by clicking "Add service" (switches to add mode) or by navigating away from the page.
...
- **FR-010**: When `card_fee_mode = 'custom'`, the service MUST also carry a non-null `card_fee_custom_cents` (integer; `0 ≤ value ≤ 5000`, i.e. capped at $50). When `card_fee_mode != 'custom'`, `card_fee_custom_cents` MUST be null. The system MUST enforce both the not-null/null pairing AND the upper cap via a database CHECK constraint AND in the Server Action's validation layer.
...
- **FR-012**: When Custom is selected, the panel MUST render a single `$` amount input beside the segmented control accepting plain decimals (`0`, `4`, `4.50`, `4.5`), formatting to two decimals on blur, rejecting negative values, AND rejecting values greater than `$50`. The input MUST be required and MUST validate to a non-negative number ≤ `$50`; "Save" MUST be disabled when empty, invalid, or above the cap. An explicit `$0` is allowed (semantic meaning "intentionally zero for this service"). When the typed value exceeds the cap, the input MUST show an inline validation hint "Card fee can't exceed $50."
...
- **FR-016**: Each service MUST optionally carry a supply deduction described by two columns: `supply_amount_cents` (integer; `0 < value ≤ 5000` when non-null, i.e. positive and capped at $50) and `supply_label` (text, nullable). The two MUST be both null or both non-null. The system MUST enforce the pairing rule, the positive-amount rule, AND the upper cap via a database CHECK constraint AND in the Server Action's validation layer.
...
- **FR-019**: When Supply is on, both inputs MUST be required: `supply_amount_cents > 0` AND `supply_amount_cents ≤ 5000` (positive, capped at $50), `supply_label` non-empty (≥ 1 character after trim). Inline validation hints MUST guide the operator: "Enter a positive amount up to $50, or turn Supply off" for the amount (with a tighter "Supply can't exceed $50" hint when the value exceeds the cap); "Add a short label so staff know what this covers, or turn Supply off" for the label. "Save" MUST be disabled until both are valid.
