...
- **FR-007**: System MUST cache a record of each gift card it encounters — at minimum a masked tail of the number (e.g., last 4 digits), its last-known available balance, the upstream provider's stable id, and the time the cached balance was last refreshed — so the salon has an auditable trail of which card was used and when. The cache is for display/history only; every active lookup performed by the staff MUST refresh from the upstream provider so the displayed balance is current.
...
- **FR-019**: When a leg fails (decline, timeout, customer cancel), system MUST show the leg as failed without marking the ticket paid, and MUST allow the staff to recover by composing a fresh leg next to it (which will be a new attempt with its own per-attempt payment record). The failed leg row itself stays in payment history for audit and reconciliation — succeeded and failed leg rows are immutable from the checkout view; the refund flow (later phase) is the only path that creates a compensating record.
