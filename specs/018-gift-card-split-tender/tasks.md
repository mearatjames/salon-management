...
- [ ] T001 Confirm working tree is on branch `018-gift-card-split-tender` and clean (run `git status` from repo root; should show only the spec dir from prior phases of /ship)
- [ ] T002 Confirm dependencies are current: `npm install` is a no-op (no new packages this feature; square@^44.0.1 already present)
...
- [ ] T033 [P] [US2] Write `tests/unit/checkout/remove-draft-leg.test.ts` — asserts `removeDraftLeg` deletes the row + emits `payment.draft_removed`; rejects with `DraftLegNotFoundError` on a non-existent or non-draft row.
- [ ] T034 [P] [US2] Write `tests/unit/checkout/activate-cash-draft.test.ts` — asserts `activateCashDraft` flips draft→succeeded atomically; runs the legs-sum-to-total guard (refuses with `LegSumMismatchError` when sum ≠ total); emits `payment.captured`; flips the ticket to paid when the activation closes it.
- [ ] T035 [P] [US2] Write `tests/unit/checkout/cart-edit-invalidates-drafts.test.ts` — asserts that calling `addServiceLine` on a ticket with 2 drafts wipes both drafts and emits 2 `payment.draft_removed` rows; the action's success-result includes `{draftsDiscarded: 2}`; succeeded legs are preserved.
- [ ] T036 [P] [US2] Write `tests/unit/checkout/one-in-flight-per-ticket.test.ts` — asserts that a second `activate*` call while one leg is `'pending'` fails on the unique partial index (`23505`); the action catches and surfaces `TicketAlreadyBeingChargedError`.
- [ ] T037 [P] [US2] Write `tests/unit/checkout/leg-sum-equals-total.test.ts` — asserts the RPC-side guard inside `pos_activate_cash_draft` and `pos_record_gift_payment`: activation refused with `legs_must_sum_to_total` when sum of non-failed legs ≠ `tickets.total_cents`.

### Implementation for User Story 2

- [ ] T038 [US2] Implement `composeDraftLeg(ticketId, method, amountCents)` Server Action in `app/(studio)/checkout/actions.ts` per [contracts/server-actions.md § 2](./contracts/server-actions.md). Calls `pos_compose_payment_draft` RPC; maps RPC-side `legs_must_fit_remaining` to `LegAmountInvalidError`; maps the in-flight refusal to `TicketAlreadyBeingChargedError`.
- [ ] T039 [US2] Implement `removeDraftLeg(paymentId)` Server Action in `app/(studio)/checkout/actions.ts` per [contracts/server-actions.md § 3](./contracts/server-actions.md). Calls `pos_remove_payment_draft` RPC; maps `draft_leg_not_found` to `DraftLegNotFoundError`.
- [ ] T040 [US2] Implement `activateCashDraft(paymentId)` Server Action in `app/(studio)/checkout/actions.ts` per [contracts/server-actions.md § 4](./contracts/server-actions.md). Calls `pos_activate_cash_draft` RPC; catches `23505` from the unique-in-flight index and surfaces `TicketAlreadyBeingChargedError`; maps RPC-side `legs_must_sum_to_total` to `LegSumMismatchError`.
- [ ] T041 [US2] Extend `sendCardToTerminal` in `app/(studio)/checkout/actions.ts` to accept the optional `{existingDraftId?: string}` option per [contracts/server-actions.md § 7](./contracts/server-actions.md). When `existingDraftId` is provided, verify the draft row matches `(ticket_id = ticketId, status='draft', method='card')`, transition to `'pending'` atomically (catch `23505` → `TicketAlreadyBeingChargedError`), then proceed with the existing Square `terminals.createCheckout` flow. Single-tender callers continue to omit the option and get the existing one-shot behavior.
...
- [ ] T044 [US2] Modify `app/(studio)/checkout/[ticketId]/page.tsx` to load the ticket's draft + pending + succeeded legs alongside the existing data and pass them to the client island.
- [ ] T045 [US2] Modify `app/(studio)/checkout/[ticketId]/checkout-screen.client.tsx` — wire the Split tile (tap → switches the cart footer to `<SplitCartFooter/>`); render the split-mode footer when there are any non-failed legs (covers auto-entry from US3's partial-gift case); intercept the existing line-mutation actions' new `draftsDiscarded` field to toast when applicable; surface `TicketAlreadyBeingChargedError` with the spec's "Ticket is already being charged on another device" copy.

### E2E for User Story 2

- [ ] T046 [US2] Write `tests/e2e/split-tender-cash-card.spec.ts` — US2 happy path per [quickstart.md § 4 Story 2](./quickstart.md). Describes "US2: split tender — cash + card". $60 ticket; compose $20 cash + $40 card; activate both; ticket paid. Audit cursor asserts: `payment.draft_created` × 2, `payment.captured` (cash), `payment.captured` (card on webhook).
- [ ] T047 [US2] Write `tests/e2e/concurrent-charge-blocked.spec.ts` — FR-022 enforcement. Two browser contexts both open the same ticket and both try to activate a cash leg; the second sees `TicketAlreadyBeingChargedError` and the current state. Describes "US2: concurrent charge blocked".

### US2 gate

- [ ] T048 [US2] Scoped Phase-4 gate: scoped prettier + scoped eslint (same `git diff` recipe as T031); full `npm run typecheck`; full `npm test`; `npx playwright test tests/e2e/split-tender-cash-card.spec.ts tests/e2e/concurrent-charge-blocked.spec.ts -g "US2"`. All must pass.
...
- [ ] T049 [P] [US3] Extend `tests/unit/checkout/redeem-gift-whole-ticket.test.ts` with the partial-balance branch — asserts that `redeemGiftCardWholeTicket` on a $40 ticket + ACTIVE $15 card → activates the gift leg for $15, composes a second draft for $25 with `method='cash'` placeholder, audit payload `{auto_split_from_gift: true, pending_method_pick: true}`, and returns `{kind: 'partial_split', paymentId, nextDraftPaymentId, nextDraftAmountCents: 2500}`.

### Implementation for User Story 3

- [ ] T050 [US3] Extend `redeemGiftCardWholeTicket` in `app/(studio)/checkout/actions.ts` with the partial-coverage branch per [contracts/server-actions.md § 6](./contracts/server-actions.md) and [research.md § R6](./research.md#r6--partial-gift-auto-split-flow-story-3--fr-006). When `balanceCents < remainingOwed`: activate the gift leg for `balanceCents`, then synthesize a second draft via `pos_compose_payment_draft(method='cash', amount=remainingOwed - balanceCents)` and include `auto_split_from_gift: true, pending_method_pick: true` in the draft's audit-row payload. Return `{kind: 'partial_split', ...}`.
- [ ] T051 [US3] Extend `components/lacquer/checkout/gift-card-balance-sheet.tsx` to render the `partial` state — "$X available · ticket needs $Y · split needed" with a "Redeem available" CTA (single tap; matches SC-003).
- [ ] T052 [US3] Extend `components/lacquer/checkout/payment-leg-row.tsx` to render the "Pick method" affordance on a leg that has `auto_split_from_gift = true` and no method picked yet (visual: a method-picker tile row inline instead of a fixed method icon). On tap of a method, the row updates the draft's method (server action `setDraftLegMethod(paymentId, method)` — small new action) and the activate button becomes enabled.
- [ ] T053 [US3] Add the small `setDraftLegMethod(paymentId, method)` Server Action in `app/(studio)/checkout/actions.ts` — UPDATEs a draft row's `method`; refuses with `DraftLegNotFoundError` on a non-draft; emits a `payment.draft_created` audit row variant or reuses the existing one (the entry already exists from the synthesized draft, so this is just an UPDATE, not a re-insert — the audit verb is `payment.draft_method_picked` ... wait — that's a new verb. To keep the audit vocabulary tight, this UPDATE emits a single audit row with action `payment.draft_method_picked` added to the AuditAction union at this point). Update [contracts/audit.contract.md](./contracts/audit.contract.md) accordingly and add the verb to T006's work (or in this task if not already done — verify and patch).
- [ ] T054 [US3] Modify `app/(studio)/checkout/[ticketId]/checkout-screen.client.tsx` — after `redeemGiftCardWholeTicket` returns `{kind: 'partial_split', ...}`, automatically open `<SplitCartFooter/>` and focus the pre-populated second leg row so the operator sees the "Pick method" prompt immediately (matches SC-003's "at most one tap").

### E2E for User Story 3

- [ ] T055 [US3] Write `tests/e2e/gift-card-partial-balance.spec.ts` — US3 happy path per [quickstart.md § 4 Story 3](./quickstart.md). Describes "US3: gift card partial balance". $40 ticket + GAN suffix `0002` (ACTIVE $15); operator redeems → cart auto-splits with $25 draft → picks cash → activates → ticket paid. Audit cursor asserts: `gift_card.balance_looked_up`, `payment.draft_created` (gift, $15), `gift_card.redeemed`, `payment.draft_created` (cash, $25, with `auto_split_from_gift: true`), `payment.draft_method_picked` (if applicable — the test asserts what actually happens), `payment.captured` (cash).

### US3 gate

- [ ] T056 [US3] Scoped Phase-5 gate: scoped prettier + scoped eslint; full `npm run typecheck`; full `npm test`; `npx playwright test tests/e2e/gift-card-partial-balance.spec.ts -g "US3"`. All must pass.
...
- [ ] T057 Confirm `CLAUDE.md`'s SPECKIT marker points at `specs/018-gift-card-split-tender/plan.md` (set during Phase 1 of /ship; sanity-check it hasn't drifted).
- [ ] T058 Run the design-system auditor against the feature branch — `speckit-design-auditor` invocation (or its agent equivalent) over the touched `components/lacquer/checkout/` and `app/(studio)/checkout/` surfaces. Must return PASS.
- [ ] T059 Run the quickstart's manual exercise checklist ([quickstart.md § 4](./quickstart.md)) against `npm run dev` — Story 1 happy path, Story 2 cash+card, Story 3 partial gift, and the four error-edge GAN suffixes. Smoke-test only; the e2e suite is the canonical regression net.
- [ ] T060 Final full gate (per CLAUDE.md § Pre-push quality gates): `npm run format:check && npm run lint && npm run typecheck && npm test && npm run test:e2e`. All five must be green before any push. The e2e step is the full suite, not scoped.
