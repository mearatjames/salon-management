...
1. **Given** the signed-in operator has exactly one open ticket they created earlier in this shift, **When** they tap "Checkout" in the sidebar, **Then** they are routed to that open ticket's checkout screen with its existing cart contents intact.
2. **Given** the signed-in operator has no open tickets, **When** they tap "Checkout" in the sidebar, **Then** a new empty ticket is created and they are routed to it.
3. **Given** the signed-in operator has more than one open ticket they created, **When** they tap "Checkout" in the sidebar, **Then** the most recently updated open ticket is resumed. (No multi-ticket picker in this phase.)
...
- **Cancel from the checkout header**: The technician can cancel out of the checkout screen. The in-progress ticket remains open (it can be resumed via the sidebar). Cancellation does not delete the ticket.
...
- **FR-003**: When the sidebar "Checkout" entry point is used, the system MUST resume the operator's single open ticket if one exists, OR create a fresh empty ticket and route to it if none does (per US2). When more than one of the operator's tickets is open, the system MUST resume the most recently updated one.
- **FR-004**: A ticket created by either entry point MUST start in an "open" state with no client and no appointment associated.
- **FR-005**: The system MUST allow the operator to cancel out of the checkout screen without paying; cancellation MUST leave the ticket in its open state (no deletion, no status change to paid).
...
- **FR-018**: Activating the cash payment with a non-zero total and no unconfirmed-price lines MUST record a cash payment for the full current total, MUST flip the ticket to a paid state, and MUST then route the operator to the confirmation screen.
- **FR-019**: The cash payment MUST NOT prompt for or capture a tip in this phase.
- **FR-020**: The cash payment MUST proceed regardless of whether a cash-drawer session has been opened in this phase. (Drawer-session gating is later scope.)

#### Confirmation and follow-up

- **FR-021**: After a successful cash payment, the checkout screen MUST render a confirmation that prominently shows the charged total ("Charged $X").
- **FR-022**: The confirmation screen MUST expose a "New sale" action that, when activated, creates a fresh empty ticket (per FR-002) and routes the operator into it.

#### Receipt

- **FR-023**: For any paid ticket, the system MUST expose a printable browser-rendered receipt that lists the salon name, the line items (with snapshotted name and price), subtotal, total charged, and payment method.
- **FR-024**: The printable receipt MUST render without studio chrome (no sidebar, no header nav) so a default browser-print produces a clean page. No external PDF library is required in this phase.

#### Visual/system constraints

- **FR-025**: All UI in this feature MUST follow the Lacquer design system: tokenized colors/spacing/radii, shadcn/ui primitives, Lucide icons, Inter typography, tabular numerals for currency. The layout MUST adapt the existing single-screen transaction prototype (do not redraw).

### Key Entities

- **Ticket**: A standalone sale record. Has a status that begins "open" and becomes "paid" once a successful payment closes it out. May optionally reference a client and an appointment (both null in this phase). Holds the running cart through its associated items, and the payment record(s) that closed it.
