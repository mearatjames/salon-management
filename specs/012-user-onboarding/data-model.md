...
| `offboarded` | `reactivateUser` | `invited` | `admin.generateLink({ type: 'magiclink', email })` (fresh token); UPDATE `state='invited'`, `active=false`, `offboarded_at=NULL`, `offboarded_by=NULL`, `offboard_reason=NULL`, `invited_at=now()`, `invited_by=<owner>`, `invite_method='magic_link'`; audit `user.reactivated`. |
