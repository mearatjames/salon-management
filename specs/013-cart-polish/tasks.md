...
- [ ] T025 [P] [US3] Write `tests/e2e/checkout-discount.spec.ts`. Describe block: `"US3: Discount lines"`. Covers US3 acceptance scenarios 1–8 plus the FR-019 status invariant and the unconfirmed-line interaction edge case: (a) `+ Discount` opens sheet with two shape options; (b) Flat amount + note "Loyalty perk" → row shows note as suffix label, negative amount, total recomputes; (c) Percent 15 → row shows "-$X.XX" computed from current service subtotal; (d) adding a new service line → percent discount amount recomputes against new subtotal; (e) remove discount via row's remove control → total recomputes back; (f) over-discount (flat $50 on a $30 cart) → displayed total floors to $0 AND Charge disabled; (g) note empty → row falls back to "Discount" (flat) or "Discount · 15%" (percent); (h) note populated → row shows the note; (i) **[FR-019 status invariant]** after `addDiscountLine` AND after `removeDiscountLine`, assert via DB read that `tickets.status='open'` — neither operation transitions the ticket; (j) **[Edge Case: discount + unconfirmed line]** add a variable service (Cancel the auto-opened price sheet so the row stays unconfirmed) AND then a flat $5 discount → the discount line lands in the cart, the displayed total reflects the discount against the confirmed service subtotal, but Charge stays disabled with the `"Set price on highlighted items"` hint (the unconfirmed-gate from US1 takes precedence over the discount floor). Assert via DB on `ticket_items` rows (`kind='discount'`, `discount_pct`, `note`) and on `audit_log` via cursor.
