...
- [ ] T048 Open a PR; verify the preview migration GitHub Action (`db-migrate-preview.yml`) applies the cart-polish migrations (`0005_add_discount_enum_value.sql` + `0006_cart_polish.sql` — split per Phase 2 deviation, not the planned single `0005_cart_polish.sql`) to the preview Supabase project and the Vercel preview deploy comes up green against it. Coordinate with any concurrent feature work that also touches `supabase/migrations/`. _Deferred to user — PR opening is for orchestrator/user, not the implement skill._
