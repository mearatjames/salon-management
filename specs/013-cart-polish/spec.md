...
- Q: When the price sheet is auto-opened for a brand-new unconfirmed variable row, what should the sheet's Cancel button do? → A: Cancel closes the sheet only; the row stays in the cart in its unconfirmed state. Removing the row is the explicit job of the per-line Remove control or the sheet's Remove button (which appears only for unconfirmed rows). Cancel and Remove are distinct in every variant of the price sheet.
- Q: How should a discount line populate `ticket_items`'s currently-NOT-NULL columns? → A: Make `ref_id` and `assigned_staff_id` nullable, with a CHECK constraint that enforces NOT NULL only when `kind = 'service'` and NULL when `kind = 'discount'`. `name_snapshot` remains NOT NULL and is populated from the discount UI for discount lines. Same table, two kinds — no sentinel service row, no separate discounts table.
- Q: Should the discount sheet capture a free-text reason / note? → A: Yes — an optional free-text note (max 80 chars) captured in the discount sheet, persisted on the discount line via a new nullable `note` text column on `ticket_items`, displayed in the cart row and on the printed/emailed bill when present. Front-desk staff regularly justify discounts at the moment they apply them ("Loyalty perk", "Re-do", "Promo"), and persisting the note keeps the audit trail meaningful for the owner reviewing closed-out tickets.
...
7. **Given** the discount sheet is open with the note input populated (e.g., "Loyalty perk"), **When** the technician saves the discount, **Then** the note is persisted on the discount line AND displayed in the discount row in the cart AND included on the discount line in the printed/emailed bill.
8. **Given** the discount sheet is open with the note input empty, **When** the technician saves the discount, **Then** the cart row falls back to a generic label ("Discount" for flat, "Discount · {pct}%" for percent) and the bill renders the same fallback.
...
- **Cancel from the price sheet (any variant)**: Cancel only closes the sheet — it never deletes the cart row. For a brand-new auto-opened variable row, Cancel leaves the row in the cart in its unconfirmed state (the operator gets the "Set price on highlighted items" hint and can come back via the price button, or delete the row via the per-line Remove control or the sheet's Remove button — both wired for unconfirmed rows). For an override session on a confirmed row, Cancel leaves the row priced as it was.
...
- **Suggested-gratuity baseline on the bill**: The three suggested-gratuity rows (18% / 20% / 25%) are computed against the pre-discount service subtotal (sum of `kind='service'` line amounts), NOT the discounted total. This matches restaurant convention — the customer reads the tip line off the pre-discount check — and matches the prototype's behavior. The bill's "Total before tip" row continues to reflect the discounted total, so the customer sees both numbers explicitly: the pre-discount tip baseline and the actually-owed total.
...
- **FR-016a**: The discount sheet MUST expose an optional free-text note input (maximum 80 characters; empty allowed). On save, the note MUST be persisted on the discount line. When present, the note MUST be displayed in the discount row in the cart (below or beside the row's amount) and in the discount line on the printed/emailed bill. When absent, the discount row MUST fall back to a generic label (e.g., "Discount" for flat amounts, "Discount · {pct}%" for percent discounts).
...
- **FR-028**: The system MUST extend `ticket_items` to support discount lines: the `kind` enum MUST add the value `discount`, the table MUST add a nullable `discount_pct` column that holds the percentage (when the discount was entered as a percent) so the amount can be recomputed against the live service-line subtotal until charge, the table MUST add a nullable `note` text column (max 80 chars, used by discount lines to carry the operator-entered reason such as "Loyalty perk"), and the `ref_id` and `assigned_staff_id` columns MUST be relaxed to nullable. A CHECK constraint MUST enforce that `ref_id` and `assigned_staff_id` are NOT NULL when `kind = 'service'` and NULL when `kind = 'discount'`. `name_snapshot` MUST remain NOT NULL for every kind and is populated from the discount UI for discount lines (e.g., "Discount", "Discount · 15%").
...
- **Ticket item (extended)**: The existing cart-line entity gains support for discount lines via a new `kind = discount` value, a new optional `discount_pct` field, and a new optional `note` text field (max 80 chars, used by discount lines to carry the operator-entered reason). The `ref_id` (service) and `assigned_staff_id` (tech) references are relaxed to nullable, with a CHECK constraint that requires them on `kind = 'service'` rows and forbids them on `kind = 'discount'` rows. Service lines continue to behave as in phase 2. Discount lines store a negative line amount and a name snapshot supplied by the discount UI; when entered as a percent, they also remember the percent so the amount remains correct after subsequent service-line edits.
...
- **Discount lines do not own a service reference**: Unlike service lines, discount lines do not reference a row in `services` and are not attributed to any specific tech. Per the clarification above, the migration relaxes `ticket_items.ref_id` and `ticket_items.assigned_staff_id` to nullable and adds a CHECK constraint that gates nullability by `kind`. The cart-totals query keeps using the same single-table read; per-tech labor reporting (a later phase) will simply exclude `kind = 'discount'` rows since they have no `assigned_staff_id`.
