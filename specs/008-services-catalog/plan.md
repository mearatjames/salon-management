# Implementation Plan: Services catalog (Settings → Services)
...
**Scale/Scope**: Single salon. Expected catalog size ≤100 services across ≤20 categories. ≤10 concurrent operator devices. Scope of this feature: one new migration, one new page + drawer + dialog + toast bridge client island, four Server Actions + one read helper, ~10 validator/helper modules, ~4 Vitest specs, one Playwright spec, an edit to `components/lacquer/settings/tab-bar.tsx` to add the "Services" tab. The Settings shell layout (`app/(studio)/settings/layout.tsx`) is **not** touched — the existing owner/manager gate already protects every settings route; this feature relaxes the gate inside the page itself so technicians and front-desk can read.
...
|---|---|---|
| **I. Design System Fidelity (NON-NEGOTIABLE)** | ✅ Pass | Every visual value resolves to a token in `styles/tokens.css`. Reuses Lacquer primitives already on disk: the Settings shell + tab bar (feature 006 / 007), `components/lacquer/staff/*` for drawer + dialog + toaster patterns, `components/ui/*` shadcn primitives, Lucide icons (Plus, Search, Eye, EyeOff, AlertCircle, Archive, ArchiveRestore, X). The two new Lacquer namespaces (`components/lacquer/services/*` and the reused `components/lacquer/settings/*`) compose existing primitives — no second component library. Acceptance includes a side-by-side comparison against `design-system/ui_kits/studio/Settings.jsx` (services list mock) and the staff drawer in browser preview. |
...
└── lacquer/
    └── settings/
        └── tab-bar.tsx                  # EDIT — append { id: "services", label: "Services", href: "/settings/services" }
...
styles/
└── settings.css                         # EDIT — add .settings-services-grid, .services-drawer, .service-list-* rules
