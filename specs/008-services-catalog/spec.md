# Feature Specification: Services catalog (Settings → Services)
...
**Independent Test**: Seed the database with at least 5 services across 2 categories (one archived). Open Settings → Services, confirm the list groups by category, the active/total summary is correct, each row shows a color swatch + duration + price + tech-count, the search filter narrows the rows by name, and the "Show archived" toggle hides/shows the archived service.

**Acceptance Scenarios**:

1. **Given** the salon has 6 services across categories "Manicure" and "Pedicure" (one Manicure service archived), **When** the owner opens Settings → Services with "Show archived" off, **Then** 5 rows appear in 2 category groups (Manicure first alphabetically), each row shows a color swatch + name + duration in minutes + price (or "From $X" for variable price) + a "{N} techs" pill, and the summary reads "5 active · 6 total".
...
1. **Given** the Add drawer is closed, **When** the owner clicks "Add service", **Then** a right-side drawer slides in with the title "Add service", a primary "Save service" button (disabled), and an empty form with default duration "30", default color "Rose", taxable on, variable price off, and no staff selected.
...
3. **Given** "Variable price" is on and both bounds are empty, **When** the owner saves, **Then** the service is persisted with `price_cents = null`, `price_from_cents = null`, `price_to_cents = null`, `variable_price = true`, and the list row reads "Variable".
...
- **Concurrent edits.** Last-write-wins on the service row; if two admins edit the same service in different tabs, the second save replaces the first. Realtime invalidation is not required for the Settings surface in v1 (consistent with the staff page).
...
- **FR-014**: On successful save, the drawer MUST close, the new service MUST appear in the list under the correct category group, the list MUST scroll the new row into view, and a toast MUST read "{name} added to the catalog".
...
#### Authorization
- **FR-029**: All authenticated operators MUST be able to reach `/settings/services` and read the catalog list and drawer contents.
...
#### Data
- **FR-033**: The migration delivered with this feature MUST create a `services` table with columns id (primary key), name, category, duration_min, price_cents (nullable; null when `variable_price` is true and no fixed price applies), color_token, taxable, active, variable_price, price_from_cents (nullable), price_to_cents (nullable), variable_price_note (nullable), created_at, and updated_at. Column types MUST match the project's existing conventions for the other Settings tables (see `staff` as the reference).
...
- **FR-037**: All page surfaces (list, drawer, dialogs, toasts) MUST use Lacquer tokens, the Settings shell layout from feature 006, the Inter type / 4px spacing scale, and the icons defined in `design-system/`. No raw hex colors, off-scale spacing, custom font weights, or emoji in chrome.
...
- This feature implements only the **Services** tab inside `/settings`. It reuses the existing Settings shell (`/settings` layout with its tab strip) shipped in feature 006-staff-management. Adding "Services" as a tab inside that shell is part of this feature's scope.
...
- Realtime sync is not required for this surface in v1 — the system design lists `services` and `staff_services` outside the realtime channels table. Last-write-wins is acceptable for concurrent admin edits, matching the staff Settings surface.
