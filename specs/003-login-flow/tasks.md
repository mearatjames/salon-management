...
- [X] T001 Install `bcryptjs` (runtime) and `@types/bcryptjs` (dev) via `npm install bcryptjs && npm install --save-dev @types/bcryptjs`. Confirm `package.json` and `package-lock.json` are updated and `npm install` is idempotent. (Used by `lib/auth/pin.ts`; research.md R3.)
- [X] T002 [P] Install `jose` (runtime) via `npm install jose`. Confirm version is ≥ 5.x in `package.json`. (Used by `lib/auth/cookie.ts` for HS256 sign/verify on the Edge runtime; research.md R2.)
- [X] T003 [P] Add five shadcn primitives via `npx shadcn@latest add input label alert dropdown-menu sonner`. Confirm the generator writes `components/ui/input.tsx`, `components/ui/label.tsx`, `components/ui/alert.tsx`, `components/ui/dropdown-menu.tsx`, and `components/ui/sonner.tsx` and does not modify any other file. `sonner` is the toast primitive needed by the soft-degrade UX (FR-015a) and the wrong-PIN inline error path. No other primitives are added in this feature (Principle V; research.md R7).
- [X] T004 [P] Create `styles/auth.css` containing only the `.auth-*` classes the auth surfaces need: `.auth-shell` (centered card on neutral background, viewport-padded), `.auth-card` (Lacquer-radius-16 card with shadow-md), `.auth-headline`, `.auth-form-row`, `.auth-form-actions`, `.auth-divider` (horizontal rule with "or" badge), `.auth-magic-link` (subordinate text-link styling), `.auth-roster` (responsive grid of staff tiles), `.auth-staff-tile` (avatar + name + role chip; tap-target sized for iPad), `.auth-keypad` (3×4 grid), `.auth-keypad-key`, `.auth-keypad-display` (4 dots), `.auth-keypad-clear`. Every property MUST resolve to a `var(--*)` from `styles/tokens.css` — no raw hex, no off-scale spacing. Constitution Principle I.
- [X] T005 [P] Update `.env.example`: (a) add the new var `NEXT_PUBLIC_GOOGLE_OAUTH_ENABLED=false` under `# --- App ---` with a one-line comment ("Set to `true` only when Google credentials are configured in the Supabase dashboard."), (b) leave `ACTING_AS_COOKIE_SECRET` as-is — it is the canonical name; the contract docs and quickstart will be aligned to it in T005a. Do **not** add `AUTH_COOKIE_SECRET` (the contract drafts used that name; we honor the existing scaffold).
- [X] T005a [P] Align the auth contract docs to the existing env-var name: in `specs/003-login-flow/contracts/cookie.contract.md`, `specs/003-login-flow/plan.md`, and `specs/003-login-flow/quickstart.md`, replace every reference to `AUTH_COOKIE_SECRET` with `ACTING_AS_COOKIE_SECRET`. No other content changes. (Documentation hygiene only — no implementation depends on this task ordering, but doing it in Setup keeps later tasks searchable for the canonical name.)
- [ ] T005b [P] Configure the Supabase Auth password policy per FR-023. (a) If `supabase/config.toml` is generated by `supabase init` (run during T008's prerequisites), add `[auth.email]\nminimum_password_length = 8` to it (no character-class options — Supabase enforces length only, which matches the NIST 800-63B rationale from Q3). (b) Document the same setting for the production Supabase project in `quickstart.md` § 1 (a one-time dashboard configuration: Authentication → Providers → Email → Minimum password length = 8). The check runs in T055 via a manual try-and-fail (attempt to seed an account with a 7-char password and confirm Supabase rejects). _(deferred — config.toml not yet generated; sub-task (b) DONE in quickstart.md § 1; will be edited during T008 or immediately after `supabase init` runs in Phase 2)_
...
- [X] T007 [P] Create `supabase/seed.sql` per `data-model.md` § 7. Insert two `auth.users` rows via `auth.users` direct insert (with `encrypted_password = crypt('tang-nails-dev', gen_salt('bf'))` from pgcrypto): `owner@tangnails.dev` and `manager@tangnails.dev`. Insert three `staff` rows: Maya Patel (`owner`, `--accent-rose`, linked to owner@), Jordan Lee (`manager`, `--accent-amber`, linked to manager@), Sam Chen (`technician`, `--accent-violet`, no `user_id`). PIN hashes: precompute `bcrypt.hashSync('1234', 11)`, `'5678'`, `'9999'` once via a tiny script and embed the literal strings in `seed.sql`. Top-of-file comment notes the PINs are dev-only and that production bootstrap is a manual SQL/Studio step.
...
- [X] T051 [US6] Extend `tests/e2e/auth.spec.ts` with a US6 block. Cases: (a) signed-in + pinned-in as Maya on `/dashboard`, open the operator menu, click "Sign out" → URL is `/login`; (b) hard reload (`page.reload()`) → URL is still `/login` (the form is rendered, not the dashboard); (c) one `device.signed_out` audit row exists with `actor_user_id` = Maya's auth.users id and `acting_as_staff_id` = Maya's staff id. (Depends on T050.)
...
- [X] T052 [P] Extend `tests/e2e/auth.spec.ts` with a soft-degrade block (FR-015a / Q1 verification): (a) sign in + pin in as Maya; (b) intercept Supabase requests and return `503` for any `/rest/v1/*` or `/auth/v1/*` call via `page.route('**/127.0.0.1:54321/**', route => route.fulfill({ status: 503 }))`; (c) reload `/dashboard` → assert the studio shell renders, the Reconnecting banner is visible (poll until `text=Reconnecting…` is in the DOM via `expect(...).toBeVisible({ timeout: 12000 })`), the operator chip shows `…` placeholder; (d) attempt the operator menu's "Switch staff" → assert a retryable error toast (NOT a redirect to `/login`); (e) restore the route handler → assert the banner disappears within ~10 s and the operator chip rebuilds with Maya's name; (f) the operator cookie is still valid (assert via `context.cookies()`).
- [X] T053 [P] Run the design-system auditor against `/login`, `/select-staff`, and the studio topbar: invoke the `speckit-design-auditor` agent with target routes `app/(auth)/login/page.tsx`, `app/(auth)/select-staff/page.tsx`, and `app/(studio)/layout.tsx`. Note: no Lacquer prototype exists for the auth surfaces (verified during plan); the auditor checks token discipline + composition rules from `components/ui/*`. Address every reported violation. (SC-007, Constitution I.) — Audit FAIL → fixed: `styles/auth.css:139` changed `var(--space-9)` (undefined token) → `var(--space-10)` (40px). Re-run is now PASS. Advisory notes on tile/keypad/card layout widths (88/160/320/420px) preserved per T035 spec compliance — left as-is.
- [X] T054 Run all four quality gates locally: `npm run typecheck && npm run lint && npm test && npm run test:e2e`. All four MUST pass. The Vitest auth suite (cookie / pin / next-url / audit / session) is non-negotiable per Constitution IV.2. Resolve any drift before requesting review. (Constitution Principle IV.) — PASS: typecheck exit 0, lint exit 0 / no warnings, Vitest 9 files / 70 tests green, Playwright 28 cases registered / 28 skipped cleanly (Supabase unreachable in current environment — clean-skip is documented).
- [ ] T055 Walk through `specs/003-login-flow/quickstart.md` end-to-end manually in a real browser: §3 happy path, §4 US1–US6 scenarios, §4 Soft-degrade scenario. Tick every assertion. Note in the PR description which scenarios were verified manually vs. covered by Playwright (Google OAuth is the one manual-only check). Capture the audit-log row count before + after the walkthrough as evidence of SC-006 coverage. **DEFERRED**: requires Docker + Supabase CLI installed and a running `supabase start`. Cannot be completed by /speckit-implement in the current environment. Run the walkthrough once Supabase is available; also run T008 (`supabase db reset` + `npx supabase gen types typescript --local > lib/db/types.ts`) to replace the hand-crafted `lib/db/types.ts` with the real generated output, and T005b (add `[auth.email] minimum_password_length = 8` to the generated `supabase/config.toml`).
...
- **Phase 2F (Layouts/chrome)**: T024 is independent. T025/T026/T027/T028
  ...
  T029a (error boundary + toast) depends only on T003 (sonner primitive).
  T030 depends on T021, T025, T026, T027, T029a.
...
- Phase 1: T002 / T003 / T004 / T005 / T005a / T005b can run in parallel after
...
- Phase 4: T035 / T036 [P]; T039 (action) → T037 (keypad imports it) → T038 (page mounts both) → T040 (e2e) all sequential.
- Phase 6: T043 / T044 / T045 [P]; T046 / T047 sequential; T046a [P] (unit test for `shouldCreateUser:false` enforcement) can run alongside T047; T048 last.
...
- `[P]` tasks touch different files. Tasks that edit the same file
  ...
  in T029 + T041 + T050; `app/(studio)/layout.tsx` is touched only by T030
  but reads T029a's error.tsx neighbor) are intentionally sequential.
...
- The Supabase Auth password policy (8-character minimum, no character-
  class rules per FR-023) is configured **per Supabase project** via
  `supabase/config.toml` (local) and the Supabase dashboard (production).
  T005b owns both edits and the manual try-and-fail verification ride-
  along is in T055. There is no app-code change for it.
