...
- The `audit_log` table already exists in the schema (per `docs/system-design.md` § Data model). This feature adds rows; it does not alter the table. Controlled-vocabulary `action` values are added to whatever shared enum/constants module the codebase uses.
