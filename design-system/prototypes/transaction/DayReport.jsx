...
function DrAllStaffView({ techStats, grand }) {
  return (
    <div className="dr-detail">
      <div className="dr-detail-head">
        ...
        <div className="dr-head-totals">
          {[
            { l: 'Gross',    v: f$(grand.subtotal),         cls: ''    },
            { l: 'Deducted', v: `−${f$(grand.totalDed)}`,  cls: 'neg' },
            { l: 'Net Pay',  v: f$(grand.net),              cls: 'pos' },
            { l: 'Tips',     v: f$(grand.tips),             cls: 'tip' },
          ].map(({ l, v, cls }) => (
            <div key={l} className="dr-htotal">
        ...
        <table className="dr-table">
          <thead>
            <tr>
              ...
              <th className="num" style={{ width: 58 }}>Clients</th>
              ...
              <th className="num ded" style={{ width: 72 }} title="−$5 per supply used (Chrome / Cat-eye / OPI / GelX)">Supply −$5</th>
              <th className="num ded" style={{ width: 62 }} title="Pedi #3–5: −$3 · Pedi #6–7: −$5">Pedi tier</th>
              <th className="num" style={{ width: 78 }}>Net Pay</th>
              <th className="num" style={{ width: 70 }}>Tips</th>
            </tr>
          </thead>
          <tbody>
            {techStats.map(t => (
              <tr key={t.id} className="dr-staff-row">
                ...
                <td className={`num dc${t.supplyFee ? ' on' : ''}`}>
                  {t.isExempt ? <span className="dr-em-dash">—</span> : fDed(t.supplyFee)}
                </td>
                <td className={`num dc${t.pediFee ? ' on' : ''}`}>
                  {t.isExempt ? <span className="dr-em-dash">—</span> : fDed(t.pediFee)}
                </td>
          ...
          <tfoot>
            <tr className="dr-foot-row">
              <td>Total</td>
              <td className="num">{grand.txCount}</td>
              <td className="num">{grand.svcCount}</td>
              <td className="num">{f$(grand.subtotal)}</td>
              <td className="num dc on">{fDed$(grand.cardFee)}</td>
              <td className="num dc on">{fDed$(grand.supplyFee)}</td>
              <td className="num dc on">{fDed$(grand.pediFee)}</td>
      ...
      <div className="dr-legend">
        ...
        <span><strong>Supply −$5</strong> for Chrome, Cat-eye, OPI, GelX — any payment.</span>
        <span className="dr-legend-sep">·</span>
        <span><strong>Pedi tier</strong> −$3 (Deluxe / Vitamin / Energy) or −$5 (Lavender / Hemp Steam).</span>
        <span className="dr-legend-sep">·</span>
...
function DrTechDetailView({ t, expandedTx, onToggle }) {
  const COLS = t.isExempt ? 6 : 9;
  ...
  return (
    <div className="dr-detail">
      <div className="dr-detail-head">
        ...
        <div className="dr-head-totals">
          {[
            { l: 'Gross',    v: f$(t.subtotal),           cls: ''    },
            ...(!t.isExempt ? [{ l: 'Deducted', v: `−${f$(t.totalDeductions)}`, cls: 'neg' }] : []),
            { l: 'Net Pay',  v: f$(t.net),                cls: 'pos' },
            { l: 'Tips',     v: f$(t.tips),               cls: 'tip' },
          ].map(({ l, v, cls }) => (
            <div key={l} className="dr-htotal">
              <div className="dr-htotal-l">{l}</div>
              <div className={`dr-htotal-v ${cls}`}>{v}</div>
          ...
          <thead>
            <tr>
              ...
              {!t.isExempt && <>
                ...
                <th className="num ded" style={{ width: 62 }} title="−$5 Chrome/Cat-eye/OPI/GelX">Supply</th>
                <th className="num ded" style={{ width: 58 }} title="Pedi tiers #3–#7">Pedi</th>
              </>}
                ...
                <React.Fragment key={tx.id}>
                  ...
                  >
                    ...
                    {!t.isExempt && <>
                      ...
                      <td className={`num dc${ded.supplyFee ? ' on' : ''}`}>{fDed(ded.supplyFee)}</td>
                      <td className={`num dc${ded.pediFee ? ' on' : ''}`}>{ded.pediFee ? `-$${ded.pediFee}` : '—'}</td>
                    </>}
                    ...
                    <tr className="dr-expand-row">
                      <td colSpan={COLS}>
                        <div className="dr-expand-inner">
                                ...
                                <div key={i} className="dr-expand-line">
                                  <span>
                                    <span className="dr-expand-type">
                                      {d.type === 'card' ? 'Card fee' : d.type === 'supply' ? 'Supply' : 'Pedi tier'}
                                    </span>
                          ...
                          {tip > 0.005 && (
                            <div className="dr-expand-sec">
                              <div className="dr-expand-ttl">Tip received</div>
                              <div className="dr-expand-line">
                                <span className="dr-expand-name">{Math.round(tx.tipPct * 100)}% — kept by tech</span>
          ...
          <tfoot>
            <tr className="dr-foot-row">
              ...
              {!t.isExempt && <>
                ...
                <td className="num dc on">{fDed$(t.supplyFee)}</td>
                <td className="num dc on">{fDed$(t.pediFee)}</td>
              </>}
              <td className="num net-cell">{f$(t.net)}</td>
              <td></td>
            </tr>
            {t.tips > 0.005 && (
              <tr>
                <td colSpan={COLS} style={{ padding: '5px 10px 8px', background: 'var(--muted)', borderBottom: '1px solid var(--border)' }}>
                  <span style={{ color: 'var(--muted-foreground)', fontSize: 11 }}>Tips (separate, kept by tech): </span>
...
function DayReport({ exemptTechs = DR_DEFAULT_EXEMPT, allTx = DR_ALL_TX }) {
  ...
  // CSV export
  const exportCSV = () => {
    const hdr = ['Tech','Exempt','Clients','Services','Gross','Card Fee','Supply Fee','Pedi Fee','Total Deductions','Net Pay','Tips'];
    const rows = techStats.map(t => [
      t.full, t.isExempt ? 'Yes' : 'No',
      t.txCount, t.svcCount,
      t.subtotal.toFixed(2), t.cardFee.toFixed(2), t.supplyFee.toFixed(2), t.pediFee.toFixed(2),
      t.totalDeductions.toFixed(2), t.net.toFixed(2), t.tips.toFixed(2),
    ]);
    rows.push(['TOTAL','',grand.txCount,grand.svcCount,
      grand.subtotal.toFixed(2),grand.cardFee.toFixed(2),grand.supplyFee.toFixed(2),grand.pediFee.toFixed(2),
    ...
    <div className="dr-app">
      ...
      <div className="dr-summary">
        ...
        <div className="dr-stat">
          ...
          <div className="dr-stat-s">Card ${grand.cardFee} · Supply ${grand.supplyFee} · Pedi ${grand.pediFee}</div>
        </div>
        <div className="dr-stat">
          <div className="dr-stat-l">Net Pay (all staff)</div>
          <div className="dr-stat-v pos">{f$(grand.net)}</div>
          <div className="dr-stat-s">After all deductions</div>
        </div>
        <div className="dr-stat last">
          <div className="dr-stat-l">Tips</div>
          <div className="dr-stat-v tip">{f$(grand.tips)}</div>
          <div className="dr-stat-s">Kept separately by each tech</div>
        </div>
      </div>
      ...
      <div className="dr-body">
        ...
        <div className="dr-left">
          ...
          <div className="dr-tech-list">
            {techStats.map(t => (
              <button
                key={t.id}
                className={`dr-tech-card${selTech === t.id ? ' on' : ''}`}
                onClick={() => setSelTech(t.id)}
              >
                {/* Row 1: avatar + name */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <DrAvatar tech={t} size={30} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--muted-foreground)' }}>
                      {t.txCount} clients · {t.svcCount} svcs
                    </div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 10, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Tips</span>
        ...
        <div className="dr-right">
          {selTechData
            ? <DrTechDetailView t={selTechData} expandedTx={expanded} onToggle={toggleExpand} />
            : <DrAllStaffView techStats={techStats} grand={grand} />
          }
        </div>
      </div>
    </div>
