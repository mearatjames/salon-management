          ...
          <div className="eh-activity-item">
              ...
              <div className="eh-activity-text">
                ...
                <div className="eh-activity-reason">
                  <span className="eh-activity-reason-lbl">Reason</span>
...
function EODAmendSheet({ count, onSave, onCancel }) {
  ...
  const [reason, setReason] = ehSt("recount");
  ...
  const canSave = changed && reason && (!detailRequired || detail.trim().length > 0);
  ...
  return (
    <div className="eh-modal-scrim" onClick={onCancel}>
      <div className="eh-amend-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="eh-amend-body" data-comment-anchor="82d926b3b2-div-422-9">
          {/* Before / after */}
          ...
          {/* Reason */}
          <div className="eh-amend-section-lbl">Why are you amending?</div>
          <div className="eh-amend-reasons">
            {AMEND_REASONS.map((r) =>
            <label key={r.id} className={`eh-amend-reason ${reason === r.id ? "sel" : ""}`}>
                <input
                type="radio" name="amend-reason"
                checked={reason === r.id}
                onChange={() => setReason(r.id)} />
              
                <span className="eh-amend-radio" />
                <span className="eh-amend-reason-lbl">{r.label}</span>
              </label>
            )}
          </div>
          ...
          <div className="eh-amend-section-lbl">
            ...
            <span className={detailRequired ? "eh-amend-req" : "eh-amend-opt"}>
              · {detailRequired ? "required" : "optional"}
            </span>
          </div>
          <textarea
            ...
            placeholder={
            detailRequired ?
            "Describe why you're amending this count…" :
            "Anything that would help an auditor a month from now."
            }
            value={detail}
        ...
        <footer className="eh-amend-foot">
          ...
          <button
            className="tx-btn"
            disabled={!canSave}
            onClick={() => onSave({ newCount: newNum, reason, detail })}
            style={{ height: 40 }}>
...
function EODCashHistory({
  ...
  const saveAmend = ({ newCount, reason, detail }) => {
    const reasonLabel = AMEND_REASONS.find((r) => r.id === reason)?.label || reason;
    setCounts((prev) => prev.map((c) => {
      ...
      return {
        ...
        amendment: {
          ...
          reason,
          detail: detail.trim() || reasonLabel
        }
      };
    }));
    setAmendOpen(false);
